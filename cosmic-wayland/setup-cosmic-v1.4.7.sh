...
chmod +x *.sh bin/* patch-control-panel-v1.4.6.py patch-control-panel-v1.4.7.py 2>/dev/null || true

if [[ -f install.sh ]]; then
  say "Normalizing install.sh to v${VERSION}..."
  sed -i 's/^VERSION=.*/VERSION="1.4.7"/' install.sh
...
if [[ -f "$control_panel" ]]; then
  if [[ -f patch-control-panel-v1.4.6.py ]]; then
    say "Applying v1.4.6 non-blocking scroll patch..."
    python3 patch-control-panel-v1.4.6.py "$control_panel"
  fi
  if [[ -f patch-control-panel-v1.4.7.py ]]; then
    say "Applying v1.4.7 background action/progress patch..."
    python3 patch-control-panel-v1.4.7.py "$control_panel"
  fi
  python3 -m py_compile "$control_panel" || warn "Control panel patches were applied but py_compile failed."
fi
...
say "Heavy actions now run in the background with a bottom-right progress window."
