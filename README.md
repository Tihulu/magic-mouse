...
|---|---:|---|---|
| `cosmic-wayland/` | `1.4.5` | Pop!_OS COSMIC / COSMIC Wayland | Stable Magic Mouse gesture setup with GUI control panel, fast startup, COSMIC workspace helper, and dock icon matching fix |
...
The COSMIC package uses a stateful `cosmic-ws` helper powered by `cos-cli`, because synthetic workspace keyboard shortcuts are unreliable on COSMIC Wayland. The v1.4.5 quick installer also patches the control panel so the window appears immediately and status checks load in the background.
